...
interface NewAnimePageProps {
  initialAnime: Anime[];
  initialHasMore: boolean;
  initialError: string | null;
}

export const getStaticProps: GetStaticProps<NewAnimePageProps> = async () => {
  try {
    const data = await fetchWithRetry('https://api.jikan.moe/v4/seasons/now?page=1');
    return {
      props: {
    ...
    console.error('Error in getStaticProps for new anime:', err);
    return {
      props: {
        initialAnime: [],
...
const NewAnimePage: NextPage<NewAnimePageProps> = ({ initialAnime, initialHasMore, initialError }) => {
  ...
  return (
    <Layout>
      <Head>
        <title>Новое аниме - AnimeList</title>
        <meta name="description" content="Список нового аниме текущего сезона" />
      </Head>

      <div className={heroStyles.hero}>
        <div className={heroStyles.heroContent}>
          <h1 className={heroStyles.heroTitle}>Новое аниме</h1>
